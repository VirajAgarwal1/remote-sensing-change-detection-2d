...
class SwinTransformer(nn.Module):
    def __init__(
        ...
        *,
        ...
        channels=3,
        head_dim=32,
        window_size=8,  ##### CHANGED window-size to 8
    ...
    def forward(
        self, img
    ):  ##### CHANGED : Removed the flattening layer and MLP layer. Also made the intermediate results output also
        x = self.stage1(img)
        out1 = x.clone()
        x = self.stage2(x)
        out2 = x.clone()
...
def swin_b_encoder(
    hidden_dim=128, layers=(2, 2, 18, 2), heads=(4, 8, 16, 32), **kwargs
):
    return SwinTransformer(hidden_dim=hidden_dim, layers=layers, heads=heads, **kwargs)


def swin_l_encoder(
    hidden_dim=192, layers=(2, 2, 18, 2), heads=(6, 12, 24, 48), **kwargs
):
    return SwinTransformer(hidden_dim=hidden_dim, layers=layers, heads=heads, **kwargs)
