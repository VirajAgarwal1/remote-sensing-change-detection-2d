...
class NeighborFeatureAggregation(nn.Module):
    def __init__(self, in_d=None, out_d=64):
        ...
        self.conv_aggregation_s2 = FeatureFusionModule(
            self.mid_d * 2, self.in_d[1], self.out_d
        )
        # scale 3
        ...
        self.conv_aggregation_s3 = FeatureFusionModule(
            self.mid_d * 3, self.in_d[2], self.out_d
        )
        # scale 4
        ...
        self.conv_aggregation_s4 = FeatureFusionModule(
            self.mid_d * 3, self.in_d[3], self.out_d
        )
        # scale 5
        ...
        self.conv_aggregation_s5 = FeatureFusionModule(
            self.mid_d * 2, self.in_d[4], self.out_d
        )
...
class TemporalFeatureFusionModule(nn.Module):
    def __init__(self, in_d, out_d):
        ...
        self.conv_branch1 = nn.Sequential(
            nn.Conv2d(
                self.in_d, self.in_d, kernel_size=3, stride=1, padding=7, dilation=7
            ),
        ...
        self.conv_branch2_f = nn.Sequential(
            nn.Conv2d(
                self.in_d, self.in_d, kernel_size=3, stride=1, padding=5, dilation=5
            ),
            nn.BatchNorm2d(self.in_d),
        )
        # branch 3
        ...
        self.conv_branch3_f = nn.Sequential(
            nn.Conv2d(
                self.in_d, self.in_d, kernel_size=3, stride=1, padding=3, dilation=3
            ),
            nn.BatchNorm2d(self.in_d),
        )
        # branch 4
        ...
        self.conv_branch4_f = nn.Sequential(
            nn.Conv2d(
                self.in_d, self.out_d, kernel_size=3, stride=1, padding=1, dilation=1
            ),
            nn.BatchNorm2d(self.out_d),
        )
...
class BaseNet(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        # fpn
        p2, p3, p4, p5, mask_p2, mask_p3, mask_p4, mask_p5 = self.decoder(
            c2, c3, c4, c5
        )
