...
def val(args, val_loader, model, epoch):
    ...
    for iter, batched_inputs in enumerate(val_loader):
        ...
        loss = (
            BCEDiceLoss(output, target_var)
            + BCEDiceLoss(output2, target_var)
            + BCEDiceLoss(output3, target_var)
...
def train(
    args, train_loader, model, optimizer, epoch, max_batches, cur_iter=0, lr_factor=1.0
):
    ...
    for iter, batched_inputs in enumerate(train_loader):
        ...
        lr = adjust_learning_rate(
            args, optimizer, epoch, iter + cur_iter, max_batches, lr_factor=lr_factor
        )
        ...
        with torch.no_grad():
            f1 = salEvalVal.update_cm(
                pr=pred.cpu().numpy(), gt=target_var.cpu().numpy()
            )

        if iter % 5 == 0:
            print(
                "\riteration: [%d/%d] f1: %.3f lr: %.7f loss: %.3f time:%.3f h"
                % (
                    iter + cur_iter,
                    max_batches * args.max_epochs,
                    f1,
                    lr,
                    loss.data.item(),
                    res_time,
...
def trainValidateSegmentation(args):
    ...
    if args.file_root == "LEVIR":
        args.file_root = (
            "/home/guan/Documents/Datasets/ChangeDetection/LEVIR-CD_256_patches"
        )
    ...
    trainDataset_main = myTransforms.Compose(
        [
            ...
            myTransforms.RandomCropResize(int(7.0 / 224.0 * args.inWidth)),
    ...
    train_data = myDataLoader.Dataset(
        "train", file_root=args.file_root, transform=trainDataset_main
    )
    ...
    val_data = myDataLoader.Dataset(
        "val", file_root=args.file_root, transform=valDataset
    )
    ...
    optimizer = torch.optim.Adam(
        model.parameters(), args.lr, (0.9, 0.99), eps=1e-08, weight_decay=1e-4
    )

    for epoch in range(start_epoch, args.max_epochs):
        lossTr, score_tr, lr = train(
            args, trainLoader, model, optimizer, epoch, max_batches, cur_iter
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--file_root", default="SYSU", help="Data directory | LEVIR | BCDD | SYSU "
    )
    parser.add_argument("--inWidth", type=int, default=256, help="Width of RGB image")
    parser.add_argument("--inHeight", type=int, default=256, help="Height of RGB image")
    parser.add_argument(
        "--max_steps", type=int, default=40000, help="Max. number of iterations"
    )
    parser.add_argument(
        "--num_workers", type=int, default=4, help="No. of parallel threads"
    )
    ...
    parser.add_argument(
        "--lr_mode", default="poly", help="Learning rate policy, step or poly"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--resume",
        default=True,
        help="Use this checkpoint to continue training | "
    ...
    parser.add_argument(
        ...
        default="",
        type=str,
        help="pretrained weight, can be a non-strict copy",
    )
    parser.add_argument(
        "--ms", type=int, default=0, help="apply multi-scale training, default False"
    )
