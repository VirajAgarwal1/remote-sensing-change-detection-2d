...
class DANet(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.backbone, _, _ = Build_Backbone(
            backbone_name,
            pretrained,
            num_band,
            output_stride,
...
class DASNet(nn.Module):
    ...
    def forward(self, x):
        ...
        out_t0_conv5_norm, out_t1_conv5_norm = self.norm(
            out_t0_conv5, 2, dim=1
        ), self.norm(out_t1_conv5, 2, dim=1)
        out_t0_fc7_norm, out_t1_fc7_norm = self.norm(out_t0_fc7, 2, dim=1), self.norm(
            out_t1_fc7, 2, dim=1
        )
        out_t0_embedding_norm, out_t1_embedding_norm = self.norm(
            out_t0_embedding, 2, dim=1
        ), self.norm(out_t1_embedding, 2, dim=1)
        return [
            [out_t0_conv5_norm, out_t1_conv5_norm],
            [out_t0_fc7_norm, out_t1_fc7_norm],
            [out_t0_embedding_norm, out_t1_embedding_norm],
        ]
