...
def FCCDN_loss_BCD(scores, labels):
    device_l = scores[0].device  # Get the device of the first tensor in 'scores'

    # Move labels to the same device as scores
    labels[0] = labels[0].to(device_l)
    labels[1] = labels[1].to(device_l)

    """ for binary change detection task"""
    criterion = dice_bce_loss().to(device_l)
    loss_change = torch.tensor(0.0, device=device_l)
    loss_aux = torch.tensor(0.0, device=device_l)

    # change loss
    loss_change = criterion(scores[0], labels[0], do_sigmoid=True)
    ...
    out1 = out1.to(device_l)
    out2 = out2.to(device_l)
    out3 = out3.to(device_l)
    out4 = out4.to(device_l)

    pred_seg_pre_tmp1 = torch.ones(out1.shape, device=device_l)
    pred_seg_pre_tmp1[out1 <= 0.5] = 0
    pred_seg_post_tmp1 = torch.ones(out2.shape, device=device_l)
    pred_seg_post_tmp1[out2 <= 0.5] = 0
    pred_seg_post_tmp1 = pred_seg_pre_tmp1.clone()

    pred_seg_pre_tmp2 = torch.ones(scores[1].shape, device=device_l)
    pred_seg_pre_tmp2[out3 <= 0.5] = 0
    pred_seg_post_tmp2 = torch.ones(scores[2].shape, device=device_l)
    pred_seg_post_tmp2[out4 <= 0.5] = 0
    pred_seg_pre_tmp2 = pred_seg_pre_tmp2.clone()
