...
def train(
    ...
    for epoch in range(start_epochs, 1 + n_epochs):
        ...
        for batch_idx, sample in enumerate(loaders["train"]):
            ...
            equal_mat = (
                out_thres == label1
            )  # Compare with label1, which is on the same device
            not_equal_mat = out_thres != label1
            train_conf_mat[0] += torch.sum(
                torch.where(out_thres == 1, equal_mat, 0)
            )  # TP
            train_conf_mat[1] += torch.sum(
                torch.where(out_thres == 0, equal_mat, 0)
            )  # TN
            train_conf_mat[2] += torch.sum(
                torch.where(out_thres == 1, not_equal_mat, 0)
            )  # FP
            train_conf_mat[3] += torch.sum(
                torch.where(out_thres == 0, not_equal_mat, 0)
            )  # FN
            batch_mcc = mcc_calc(train_conf_mat)

            print(
                "Epoch: {:4d}/{:4d}\tBatch: {:4d}/{:4d}\tTraining Loss: {:.3f}\tTraining MCC: {:.3f}\t\t\t\t".format(
                    epoch,
                    n_epochs,
                    batch_idx + 1,
                    len(loaders["train"]),
        ...
        for batch_idx, sample in enumerate(loaders["val"]):
            ...
            not_equal_mat = out_thres != label1
            valid_conf_mat[0] += torch.sum(
                torch.where(out_thres == 1, equal_mat, 0)
            )  # TP
            valid_conf_mat[1] += torch.sum(
                torch.where(out_thres == 0, equal_mat, 0)
            )  # TN
            valid_conf_mat[2] += torch.sum(
                torch.where(out_thres == 1, not_equal_mat, 0)
            )  # FP
            valid_conf_mat[3] += torch.sum(
                torch.where(out_thres == 0, not_equal_mat, 0)
            )  # FN
            batch_mcc = mcc_calc(valid_conf_mat)

            print(
                "Epoch: {:4d}/{:4d}\tBatch: {:4d}/{:4d}\tValidation Loss: {:.3f}\tValidation MCC: {:.3f}\t\t\t\t".format(
                    epoch,
                    n_epochs,
                    batch_idx + 1,
                    len(loaders["val"]),
        ...
        print(
            "Epoch: {} \tTraining Loss: {:.6f} \tValidation Loss: {:.6f} \tTrain MCC: {:.6f} \tValid MCC: {:.6f}".format(
                epoch, train_loss, valid_loss, train_mcc, valid_mcc
            )
        )
...
model, optimizer, start_epoch, valid_loss_min = load_ckp(
    training_model_save_path + "/checkpoint/current_checkpoint.pt", model, optimizer
)
