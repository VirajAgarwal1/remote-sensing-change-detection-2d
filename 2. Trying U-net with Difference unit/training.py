...
def train(
    ...
    for epoch in range(start_epochs, 1 + n_epochs):
        ...
        for batch_idx, sample in enumerate(loaders["train"]):
            # move to GPU
            if use_cuda:
                sample["img_A"], sample["img_B"], sample["label"] = (
                    sample["img_A"].cuda(),
                    sample["img_B"].cuda(),
                    sample["label"].cuda(),
                )
            ...
            train_conf_mat[2] += torch.sum(
                torch.where(output == 1, not_equal_mat, 0)
            )  # FP
            train_conf_mat[3] += torch.sum(
                torch.where(output == 0, not_equal_mat, 0)
            )  # FN
            batch_mcc = mcc_calc(train_conf_mat)

            print(
                "Epoch: {}/{}\tBatch: {}/{}\tTraining Loss: {:.3f}\tTraining MCC: {:.3f}\t\t\t\t".format(
                    epoch,
                    n_epochs,
                    batch_idx + 1,
                    len(loaders["train"]),
        ...
        for batch_idx, sample in enumerate(loaders["test"]):
            ...
            print(
                "Epoch: {}/{}\tBatch: {}/{}\tValidation Loss: {:.3f}\tValidation MCC: {:.3f}\t\t\t\t".format(
                    epoch,
                    n_epochs,
                    batch_idx + 1,
                    len(loaders["test"]),
            ...
            if batch_idx % 20 == 0:
                writer.add_scalar(
                    "Valid Loss",
                    valid_loss,
                    epoch * (len(loaders["test"])) + batch_idx + 1,
                )
                writer.add_scalar(
                    "Valid MCC",
                    batch_mcc,
